...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	for (int i = 0; adapterDesc.Description[i] != NULL; i++)
	{
		fputc(adapterDesc.Description[i], f);
	}
	...
	//백 버퍼 포인터로 RanderTargetView를 생성한다
	...
	// 설정된 깊이 버퍼 구조체를 사용하여 깊이 버퍼 텍스쳐를 생성합니다.
...
{
	// 종료 전 윈도우 모드로 설정하지 않으면 SwapChain을 해제 할 때 예외가 발생합니다.
	if (m_swapChain)
	{
