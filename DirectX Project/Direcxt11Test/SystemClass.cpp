...
void SystemClass::Run()
{
	...
	// 사용자로부터 종료 메시지를 받을때까지 메시지류프를 돕니다.
	while (true)
	{
...
LRESULT CALLBACK SystemClass::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM Iparam)
{
	switch (umsg)
	{
		//키보드가 눌러졌는가 처리
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	wc.lpszClassName = m_applocationName;
	...
	if (FULL_SCREEN)
	{
		...
		//풀스크린 모드로 지정했다면 모니터 화면 해상도를 데스크톱 해상도로 지정하고 생상을 32bit로 지정한다.
	...
	// 윈도우를 생성하고 핸들을 구합니다.
	m_hwnd = CreateWindowEx(WS_EX_APPWINDOW, m_applocationName, m_applocationName,
...
void SystemClass::ShutdownWindows()
{
	...
	// 프로그램 인스턴스를 제거합니다.
	UnregisterClass(m_applocationName, m_hinstance);
	m_hinstance = 0;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT umessage, WPARAM wparam, LPARAM Iparam)
{
	switch (umessage)
	{
		//윈도우 종료를 확인합니다.
		...
		default:
		{
			return ApplicationHandle->MessageHandler(hwnd, umessage, wparam, Iparam);
		}
	}
}